...
trait PatternTypers {
  ...
  trait PatternTyper {
    ...
    private def convertToCaseConstructor(tree: Tree, caseClass: Symbol, ptIn: Type): Tree = {
      ...
      newTyper(ctorContext, useContextMirror = false).infer.inferConstructorInstance(tree1, caseClass.typeParams, ptSafe)
    ...
    def doTypedUnapply(tree: Tree, fun0: Tree, fun: Tree, args: List[Tree], mode: Mode, pt: Type): Tree = {
      ...
      def freshUnapplyArgType(): Type = {
        ...
        val pattp = newTyper(unapplyContext, useContextMirror = false).infer.inferTypedPattern(tree, unappFormal, pt, canRemedy)
      ...
      def makeTypedUnApply() = {
        ...
        //TODO-REFLECT changed to use in reflect
        //originally was: alignPatterns(fun1, args).unexpandedFormals
        val formals        = patmat.patternsUnexpandedFormals(fun1, args)
