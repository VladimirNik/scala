...
//        (phase.erasedTypes && varsym.isValue && !varsym.isMethod)) {
      ...
      def typedNew(tree: New) = {
        ...
        val r = if (sym.isAbstractType || sym.hasAbstractFlag)
      ...
      //TODO-REFLECT: here is a problem
      def typedSelectInternal(tree: Tree, qual: Tree, name: Name): Tree = {
        val patternOfTree = "New(Select(Select(Ident(java), java.lang), java.lang.String))"
        val sr = showRaw(qual)
        val printInside = patternOfTree == sr
        ...
        if (neresym) {
          ...
          //TODO-REFLECT - problem is somewhere here
