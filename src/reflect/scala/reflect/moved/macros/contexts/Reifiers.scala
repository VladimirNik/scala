...
trait Reifiers {
  ...
  def reifyTree(universe: Tree, mirror: Tree, tree: Tree): Tree = {
    ...
    val result = scala.reflect.moved.reify.`package`.reifyTree(self.universe)(callsiteTyper, universe, mirror, tree)
  ...
  def reifyType(universe: Tree, mirror: Tree, tpe: Type, concrete: Boolean = false): Tree = {
    ...
    val result = scala.reflect.moved.reify.`package`.reifyType(self.universe)(callsiteTyper, universe, mirror, tpe, concrete)
    logFreeVars(enclosingPosition, result)
    result
  }

  def reifyRuntimeClass(tpe: Type, concrete: Boolean = true): Tree =
    scala.reflect.moved.reify.`package`.reifyRuntimeClass(universe)(callsiteTyper, tpe, concrete = concrete)

  def reifyEnclosingRuntimeClass: Tree =
    scala.reflect.moved.reify.`package`.reifyEnclosingRuntimeClass(universe)(callsiteTyper)
  ...
  private def logFreeVars(position: Position, reification: Tree): Unit = {
    object utils extends {
      val global: self.global.type = self.global
      val typer: global.analyzer.Typer = self.callsiteTyper
    } with scala.reflect.moved.reify.utils.Utils
    import utils._
