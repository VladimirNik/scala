 ...
 */
class JavaUniverse extends InternalSymbolTable with JavaUniverseForce with ReflectSetup with TypecheckerImpl with RuntimeSymbolTable { self =>

  override def inform(msg: String): Unit = log(msg)
  def picklerPhase = SomePhase
  def erasurePhase = SomePhase
  //TODO-REFLECT now settings are in TypecheckerImpl (inside Mirrors)
